...
module.exports = {
  ...
  /**
     ...
     * @param {string} routeDirectory Path to the directory containing the route files,
     * defaults to `@/routes`
     * @returns {routeEntry} Route objects suitable to apply to any express Router or
     * application instance by {@link applyRoutes}
     */
  loadRoutes(routeDirectory = '@/routes') {
    const resolvedRouteDirectory = this.resolveRootAlias(routeDirectory);

    // Ok, we iterater over all methods directories
    if (!fs.existsSync(resolvedRouteDirectory)) {
      throw new Error(`Could not find directory \`${resolvedRouteDirectory}\` containing routes`);
    }

    // Get a list of files in the schemataDirectory
    const routeFilesByMethodAndParameter = recursiveRead(resolvedRouteDirectory);
    ...
    cleanedRoutesPath.forEach((rf) => { // Split it
      ...
      const requirePath = path.join(resolvedRouteDirectory, routeFilesByMethodAndParameter[i]);
      ...
      const pathParts = parts.filter((e) => e !== 'index').splice(1, parts.length - 1);
      ...
      if (!route.parametrizePath) { // If we deal with more than just one segment
        if (pathParts.length > 1) {
          absolutePath = `/${
            pathParts.map((e) => e).splice(0, pathParts.length - 1).join('/')
          }/:${
            pathParts.map((e) => e).splice(pathParts.length - 1, pathParts.length - 1)
          }`;
    ...
    return results.sort(
      ...
      (b, a) => ((a.priority > b.priority) ? 1 : ((b.priority > a.priority) ? -1 : 0)),
    );
  },
  ...
  applyRoutes(appOrRouter, routes) {
    routes.forEach((routeItem) => { // Use the routes on the appropriate methods
      appOrRouter[routeItem.method](routeItem.route.path ? routeItem.route.path : routeItem.path,
        routeItem.route.middlewares ? routeItem.route.middlewares : [],
        routeItem.route.callback ? routeItem.route.callback : async (request, response, next) => {
          response.json({
            message: `Default for ${
              routeItem.path
            }`,
  ...
  /**
     * Loads routes from directory and applies them to express application or router object,
     * see {@link loadRoutes} and {@link applyRoutes}
  ...
  loadAndApplyRoutes(appOrRouter, routesDirectory = '@/routes') {
    if (!appOrRouter) {
      throw new Error('appOrRouter argument needs to be supplied');
    }
  ...
  /**
     ...
     * @example const packageJsonPath = plumber.resolveRootAlias(`@/package.json`)
     */
  resolveRootAlias(aliasPath) {
    if (aliasPath.includes('@/')) {
      return aliasPath.replaceAll('@', this.appRootDir());
    }
  ...
  /**
     ...
     * @param {Array.<string>} [exclusionList] Array of file names exclusions,
     * defaults to `['index.js']`
     * @param {Boolean} [asArray] Return middlewares as an array instead of an
  ...
  loadMiddlewares(libPath = '@/middlewares', exclusionList = ['index.js'], asArray = false) {
    const resolvedLibPath = this.resolveRootAlias(libPath);
    // Make sure the directory exists
    if (!fs.existsSync(resolvedLibPath)) {
      throw new Error(`Could not find directory \`${resolvedLibPath}\` containing the required files`);
    }

    // Get a list of files in the schemataDirectory
    const files = fs.readdirSync(resolvedLibPath).filter((f) => !exclusionList.includes(f));
    ...
    files.forEach((file) => { // Get a model name
      ...
      const middleware = require(path.join(resolvedLibPath, middlewareFileName));
